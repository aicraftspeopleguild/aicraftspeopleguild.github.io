...
> Every SVG on this page is regenerated by `.github/workflows/heartbeat.yml` on every push to main + every 15 minutes, reading the live public API, the `state.db` runtime store, and a dynamic tag database backed by **GitHub Issues**. The badges above are clickable — the first four file a pre-filled issue that the [`cmd` workflow](.github/workflows/cmd.yml) runs immediately.
