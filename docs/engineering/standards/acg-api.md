...
All three are built by `guild/web/scripts/api/build-api.py` from the SQLite DB (`guild/web/database/acg.db`) which is itself seeded from UDT instances. Each build writes to `guild/web/api/` and is served as-is from GitHub Pages.
